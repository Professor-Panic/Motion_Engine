...
void EngineDrawTextLines(WordPath paths,float fontSize,Vector2 position,Color color){
    Vector2 offset=Vector2Zero();
        for(auto& path:paths.wordpaths){
            for(auto&points:path.geometry){

                Vector2 p=ToScreenSpace(Vector2Scale(points.p0+offset,fontSize)+position,raster_size);
                Vector2 p1=ToScreenSpace(Vector2Scale(points.p1+offset,fontSize)+position,raster_size);
                Vector2 c=ToScreenSpace(Vector2Scale(points.c+offset,fontSize)+position,raster_size);
                if(points.isCurve){
